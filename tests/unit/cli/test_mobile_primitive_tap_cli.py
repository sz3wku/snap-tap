...
class _Tapper:
    ...
    def __init__(self, result: DriverTap) -> None:
        self.result = result
...
def test_primitive_tap_success_outputs_receipt_json(tmp_path: Path) -> None:
...
def test_primitive_tap_missing_signature_file_fails_before_executor(
    tmp_path: Path,
) -> None:
    executor = FakeExecutor("success")
    app = _build_app(executor)
...
def test_primitive_tap_invalid_signature_file_fails_before_executor(
    tmp_path: Path,
) -> None:
    signature_file = tmp_path / "bad.json"
    signature_file.write_text('{"schema_version":"wrong"}', encoding="utf-8")
    executor = FakeExecutor("success")
    app = _build_app(executor)
...
def test_primitive_tap_bad_serial_fails_before_executor(tmp_path: Path) -> None:
    signature_file = _write_signature(tmp_path)
    executor = FakeExecutor("success")
    app = _build_app(executor)
...
def test_primitive_tap_resolution_blocked_returns_receipt(tmp_path: Path) -> None:
    signature_file = _write_signature(tmp_path)
    app = _build_app(FakeExecutor("blocked"))
